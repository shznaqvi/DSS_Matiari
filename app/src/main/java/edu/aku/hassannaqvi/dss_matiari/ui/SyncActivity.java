...
//Do something with the JSON string
                    if (result != null) {
                        if (result.length() > 0) {
                            downloadTables.get(position).setstatus("In Progress");
                            downloadTables.get(position).setstatusID(0);
                            downloadTables.get(position).setmessage("Inserting...");
                            ...
                            //DssRoomDatabase db = new DatabaseHelper(SyncActivity.this);
...
//                                        insertCount = db.syncUser(jsonArray);
                                        ...
                                        insertCount = db.usersDao().syncUser(jsonArray);
                                ...
                                *//*case VersionTable.TABLE_NAME:
                                ...
                                case TableVillage.TABLE_NAME:
                                    try {
                                        ...
                                        insertCount = db.VillagesDao().syncVillages(jsonArray);
                                    } catch (JSONException e) {
                                        e.printStackTrace();
                                        downloadTables.get(position).setstatus("Process Failed");
                                        downloadTables.get(position).setInfo("Time: " + time + "/" + getTime() + "\t Size: " + size);
                                ...
                                case MaxHhnoTable.TABLE_NAME:
                                    try {
                                        ...
                                        insertCount = db.MaxHHNoDao().syncMaxHHNo(jsonArray);
                                    } catch (JSONException e) {
                                        e.printStackTrace();
                                        downloadTables.get(position).setstatus("Process Failed");
                                        downloadTables.get(position).setInfo("Time: " + time + "/" + getTime() + "\t Size: " + size);
                                ...
                                case TableFollowUpsSche.TABLE_NAME:
                                    try {
                                        ...
                                        insertCount = db.FollowUpsScheDao().syncFollowUpsSche(jsonArray);
                                    } catch (JSONException e) {
                                        e.printStackTrace();
                                        downloadTables.get(position).setstatus("Process Failed");
                                        downloadTables.get(position).setstatusID(1);
        ...
        if (sdDir.exists()) {
            ...
            if (files.length > 0) {
                ...
                for (int i = 0; i < fcount; i++) {
                    TextView textView = new TextView(edu.aku.hassannaqvi.dss_matiari.ui.SyncActivity.this);
                    ...
                    handler.postDelayed(new Runnable() {
                        ...
                        public void run() {
                            ...
                            WorkManager.getInstance(SyncActivity.this).getWorkInfoByIdLiveData(photoUpload.getId())
                                    .observe(edu.aku.hassannaqvi.dss_matiari.ui.SyncActivity.this, new Observer<WorkInfo>() {
                                        ...
                                        public void onChanged(@Nullable WorkInfo workInfo) {
                                            ...
                                            if (workInfo.getState() == WorkInfo.State.FAILED) {
                                                Toast.makeText(edu.aku.hassannaqvi.dss_matiari.ui.SyncActivity.this, "Failed", Toast.LENGTH_SHORT).show();
